...
import { HomeIcon, ListIcon, MosqueIcon, HeartIcon, UsersIcon, TargetIcon } from "./icons";
...
// left-to-right screen sequence: الأهداف، الزكاة، الجمعيات، أثرك، المعاملات، الرئيسية —
// "الأهداف" (/goal) sits directly after "الزكاة", matching the desktop
// sidebar's ordering. "الجمعيات" (/charities, the cause list) and "أثرك"
// (/impact, the personal impact dashboard) are separate features and must
// not share a route.
const TABS = [
  ...
  { href: "/goal", label: "الأهداف", icon: TargetIcon },
];
          ...
          {TABS.map((tab) => {
            ...
            return (
              <Link
                key={tab.href}
                href={tab.href}
                className="relative flex min-w-[64px] shrink-0 flex-1 flex-col items-center gap-1 rounded-2xl px-2 py-2 transition-transform active:scale-90"
              >
                <span
                ...
                <span
                  className={`whitespace-nowrap text-[10.5px] font-semibold transition-colors ${
